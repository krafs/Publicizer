...
`MemberPattern` is matched against the full `Namespace.Type.Member` name wherever it sits, scope or assembly — scoping a pattern changes which types it is consulted for, not what string it sees. So `^` anchors to the start of the namespace, not the member: inside `Namespace="Combat"`, the pattern `^Damage` matches nothing and `^Combat` matches everything.
